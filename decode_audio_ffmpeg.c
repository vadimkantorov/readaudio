	...
	while (ret >= 0)
	{
		...
		if (ret == 0)
		{
			...
			while (filtering)
			{
				...
				if (ret < 0)
					goto end;
				process_output_frame(data, filt_frame, frame->nb_samples, av_ctx->channels, data_len, itemsize);
...
struct DecodeAudio decode_audio(const char* input_path, int probe, struct DecodeAudio input_hint, struct DecodeAudio output_hint, const char* filter_string)
{
	...
	if(input_path == NULL)
	{
		...
		cursor.base = cursor.ptr  = (uint8_t*)input_hint.data.dl_tensor.data;
    	cursor.size = cursor.left = nbytes(&input_hint);
	...
	if(filter_string != NULL)
	{
		...
		snprintf(buffersrc_args, sizeof(buffersrc_args), "time_base=%d/%d:sample_rate=%d:sample_fmt=%s:channel_layout=0x%"PRIx64, dec_ctx->time_base.num, dec_ctx->time_base.den, dec_ctx->sample_rate, av_get_sample_fmt_name(dec_ctx->sample_fmt), dec_ctx->channel_layout);
		...
		if (avfilter_graph_create_filter(&buffersink_ctx, buffersink, "out", NULL, NULL, graph) < 0)
		{
	...
	//duration = stream->time_base.num * (int)stream->duration / stream->time_base.den;
	double duration = fmt_ctx->duration / (float) AV_TIME_BASE;
	assert(duration > 0);
	audio.sample_rate = dec_ctx->sample_rate;
	audio.num_channels = dec_ctx->channels;
	audio.num_samples  = duration * audio.sample_rate;
	...
	uint64_t data_len = 0;
	if(output_hint.data.dl_tensor.data)
	{
		data_len = nbytes(&output_hint);
		audio.data.dl_tensor.data = output_hint.data.dl_tensor.data;
	}
	else
	{
...
int main(int argc, char **argv)
{
	...
	struct DecodeAudio input_hint = { 0 }, output_hint = { 0 };
	input_hint.data.dl_tensor.data = &buf;
	input_hint.data.dl_tensor.ndim = 1;
	input_hint.data.dl_tensor.shape = &read;
	input_hint.data.dl_tensor.dtype.lanes = 1;
	input_hint.data.dl_tensor.dtype.bits = 8;
	input_hint.data.dl_tensor.dtype.code = kDLUInt;
	
	struct DecodeAudio audio = decode_audio(NULL, false, input_hint, output_hint, argc == 4 ? argv[3] : NULL);
